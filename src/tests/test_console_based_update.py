...
def test_console_based_order_book_update(capsys):
    ...
    buy_order = {
        ...
        "ticker": "AAPL",
        "quantity": 10.0,  # Ensure float type for consistency
        "price": 150.0,
        "order_type": "limit",
        "action": "buy",
    ...
    sell_order = {
        "account_id": "account_1",
        "ticker": "AAPL",
        "quantity": 5.0,  # Ensure float type for consistency
        "price": 155.0,
        "order_type": "limit",
        "action": "sell",
        "timestamp": datetime.now()
    ...
    # Updated to match the actual output with floating-point values
    assert "Account account_1 wants to buy 10.0 at 150.0" in captured_buy.out, "Buy order not displayed."
    ...
    # Updated to match the actual output with floating-point values
    assert "Account account_1 wants to sell 5.0 at 155.0" in captured_sell.out, "Sell order not displayed."
